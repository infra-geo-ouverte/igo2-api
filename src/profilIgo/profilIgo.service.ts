...
export class ProfilIgoService {
  ...
  public async update(profilName: string, profilIgo: IProfilIgo): Promise<{ name: string }> {
    return await ProfilIgo.update(profilIgo, {
      where: {
        name: profilName
      }
    }).then((count) => {
  ...
  public async delete(profilName: string): Promise<void> {
    return await ProfilIgo.destroy({
      where: {
        name: profilName
      }
    }).then((count) => {
  ...
  public async get(): Promise<IProfilIgo[]> {
    return ProfilIgo.findAll({ order: ['id'] }).then((profilsIgo: ProfilIgo[]) => {
      return profilsIgo.map((profil) => ObjectUtils.removeNull(profil.get()));
    });
  }

  public async getById(profilName: string): Promise<IProfilIgo> {
    return ProfilIgo.findOne({
      where: {
        name: profilName
      }
    }).then((profilIgo) => {
  ...
  public async getByProfils(profils: string[]): Promise<IProfilIgo[]> {
    return ProfilIgo.findAll({
      where: {
        name: profils
      },
      order: [['id', 'DESC']]
