...
export function isOgcPushButton(selector: Selector | OgcPushButton): selector is OgcPushButton {
...
export function isLogicalArray(filters: LogicalArrayOptions | AnyBaseOgcFilterOptions): filters is LogicalArrayOptions {
  return 'filters' in filters && 'logical' in filters;
}

export function isFilterAttributeOptions(
  filters: AnyBaseOgcFilterOptions
): filters is Exclude<AnyBaseOgcFilterOptions, OgcFilterConditionsArrayOptions | OgcFilterSpatialOptions> {
...
export function searchFilter(
  ...
  if (isLogicalArray(filters)) {
    if (Array.isArray(filters.filters)) {
      return filters.filters.find((filter) => searchFilter(filter, propertyName));
    } else {
